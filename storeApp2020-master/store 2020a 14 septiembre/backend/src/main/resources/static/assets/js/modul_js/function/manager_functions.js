...
API.serverResponse = function(response){
     ...
     if(Array.isArray(response)){
          if(response[0].error == 0){
                ...
                location.reload();
                  ...
                  for (let i=0; i<response.length;i++){
                    if(response[i].error == 610){
                      $(response[i].idButton).style.display= "none";
                      $(response[i].idErrorBox).style.display= "block";
                      let x=response[i].tiempoBloqueo;
                      let intervalo = setInterval(() => {

                        x--;
                        if (x == 0) {
                          $(response[i].idButton).style.display= "block";
                          $(response[i].idErrorBox).style.display= "none";
                          clearInterval(intervalo);
                        }
                    
                     $(response[i].idErrorBox).innerHTML="Estas bloqueado, te quedan: "+x+" segundos."
                      }, 1000);
                   //location.reload();
                 } else{
              ...
              $("boxerror" + control).innerHTML = response[i].messageErrorControl;
              $("boxerror" + control).style.display = "block";
              if ($("boxinfo" + control)) {
                $("boxinfo" + control).style.display = "none";
              }    
           }                   
           }   
       }                 
